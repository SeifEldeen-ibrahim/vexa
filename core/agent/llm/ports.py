...
def close_event_stream(events: object) -> None:
    ...
    — the caller stopped reading the process without stopping it, and the worker stayed exactly as
    busy as before. The identical tree passed in 0.67 s on 3.12.13. Both interpreters satisfy
...
def run_harness_turn(
    ...
    *,
    ...
    # may stop reading this turn mid-stream, and the harness generator underneath owns the CLI
    # subprocess. See `close_event_stream`.
